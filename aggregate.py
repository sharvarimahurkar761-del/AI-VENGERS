...
class Aggregator:
    ...
    def kmeans_cluster(data, k=4, iters=10, n_init=10):
        ...
        for _ in range(n_init):
            ...
            indices = np.random.choice(len(data), size=min(k, len(data)), replace=False)
    ...
    def generate_report(store):
        ...
        features_list = ['usage_decline', 'negative_sentiment', 'support_delay', 'onboarding_gap']
        ...
        for i in range(4):
            ...
            avg_outcome = np.mean([outcome_map.get(records[idx]['user_id'], 0.0) for idx in cluster_indices])
            
            report_lines.append(f"{pct:.0f}% of users — dominant cause: {dominant_cause} — recommended action: {most_common_action} — avg outcome: {avg_outcome:+.2f} engagement")
