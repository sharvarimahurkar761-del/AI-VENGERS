...
def train_agent(episodes=1500):
    ...
    for i in range(episodes):
        ...
        context_vec = agent.extract_features(user_data['risk_profile'], user_data['kg_response'])
        chosen_action = agent.choose_action(context_vec)
        
        reward, metrics = RewardFunction.calculate_reward(chosen_action, user_data['best_action'])
        
        deliverer.deliver(user_data['user_id'], chosen_action, user_data['risk_profile'])
