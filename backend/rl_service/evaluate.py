...
def run_evaluation():
    ...
    user1 = simulator.generate_user_episode(force_cause='usage_decline', force_risk_score=0.8)
    user2 = simulator.generate_user_episode(force_cause='support_delay', force_risk_score=0.8)
    ...
    if len(store.action_log) > 0:
        ...
        payload = FeedbackLoop.generate_feedback_payload(sample_action, sample_outcome)
        print(payload)
    ...
    for _ in range(100):
        ...
        ambiguous_profile['attributions'] = [
            ...
            {"feature": "negative_sentiment", "impact": random.uniform(0.3, 0.8)},
        ...
        ambiguous_action_counts[action] = ambiguous_action_counts.get(action, 0) + 1
    ...
    if incentive_pct > 10 and human_handoff_pct > 10:
        print(f"YES: The agent gracefully splits decisions between incentive ({incentive_pct}%) and human_handoff ({human_handoff_pct}%), matching the ambiguous causes.")
    else:
