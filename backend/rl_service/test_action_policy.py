...
def test_aggregation_finds_all_known_causes(trained_system):
    ...
    for _ in range(3): # Run 3 times with different random seeds implicitly
        ...
        assert len(lines) == 4, f"Expected exactly 4 clusters in report, got {len(lines)}"
...
def test_feedback_payload_schema(trained_system):
    ...
    payload_str = FeedbackLoop.generate_feedback_payload(sample_action, sample_outcome)
    ...
    assert len(set([outcome["engagement_delta"], outcome["ticket_volume_delta"], outcome["satisfaction_delta"]])) > 1, "Metrics are identical, independent noise not working"
