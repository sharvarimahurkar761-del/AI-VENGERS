...
class Simulator:
    ...
    def generate_user_episode(self, force_cause=None, force_risk_score=None):
        ...
        risk_profile = self.risk_client.get_risk_score(
            user_id, _injected_cause=true_cause)
        ...
        kg_response = self.kg_client.get_knowledge_response(
            user_id, issue_text)
...
class RewardFunction:
    ...
    def calculate_reward(chosen_action, best_action):
        ...
        reward = (engagement_delta + satisfaction_delta -
                  ticket_volume_delta) / 3.0
