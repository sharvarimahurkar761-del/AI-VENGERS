...
def run_evaluation():
    ...
    for i, item in enumerate(eval_data, 1):
        ...
        stats["count"] += 1
        if eval_metrics.get("grounded", False):
            stats["grounded_count"] += 1
        if eval_metrics.get("hallucination", False):
    ...
    for cat, stats in results_by_category.items():
        ...
        lines.append(
            f"- **Grounded Rate**: {(stats['grounded_count']/stats['count'])*100:.1f}%")
        lines.append(
            f"- **Hallucination Rate**: {(stats['hallucination_count']/stats['count'])*100:.1f}%")
        lines.append(
            f"- **Avg Confidence**: {stats['total_confidence']/stats['count']:.2f}\n")

    lines.append(f"### Overall")
    lines.append(f"- **Total Queries**: {total_queries}")
    lines.append(
        f"- **Overall Grounded Rate**: {(total_grounded/total_queries)*100:.1f}%")
    lines.append(
        f"- **Overall Hallucination Rate**: {(total_hallucinated/total_queries)*100:.1f}%")
    lines.append(
        f"- **Overall Avg Confidence**: {total_conf/total_queries:.2f}\n")
    ...
    for item in eval_data:
        ...
        lines.append(
            f"**Grounded:** {item['eval'].get('grounded')} | **Hallucination:** {item['eval'].get('hallucination')}")
