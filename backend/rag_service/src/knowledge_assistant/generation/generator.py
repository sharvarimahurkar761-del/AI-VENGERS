...
class Generator:
    def __init__(self):
        ...
        self.is_mock = not api_key or api_key == "dummy"
    ...
    def generate_response(self, query: str, retrieved_chunks: List[Dict[str, Any]], confidence_threshold: float = 0.4) -> Dict[str, Any]:
        ...
        top_score = retrieved_chunks[0].get(
            "score", 0.0) if retrieved_chunks else 0.0
        ...
        if getattr(self, "is_mock", False):
            response_text = "This is a mock RAG response because no GROQ_API_KEY was provided. According to the documentation, this issue has been resolved in the latest patch."
            if retrieved_chunks:
                response_text += f" [doc_1]"
        else:
        ...
        for citation in bracket_citations:
            ...
            if not citation_matched:
                print(
                    f"WARNING: Citation validation failed. LLM fabricated citation: [{citation}]")
